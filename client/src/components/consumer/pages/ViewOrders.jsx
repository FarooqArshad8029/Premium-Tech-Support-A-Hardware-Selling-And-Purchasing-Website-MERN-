...
import { getUserOrderDetails } from '../../../redux/actions/orderAction';
                  ...
                  <div className="mb-4">
                    ...
                    {order.products.map((product) => (
                      <div key={product?.product?._id} className="border rounded-lg p-4 mb-4 flex flex-col md:flex-row items-center">
                        {product.product && product.product.images && product.product.images.length > 0 && (
                          <img
                            src={product.product.images[0].url}
                            alt={product.product.name}
                            className="w-full md:w-24 h-24 object-cover rounded-lg mb-4 md:mr-4 md:mb-0"
                          />
                        )}
                        <div>
                          <p className="font-bold">{product.product && product.product.name}</p>
                          <p className="text-gray-300">Price: ${product.product && product.product.price}</p>
                          <p className="text-gray-300">Quantity: {product.quantity}</p>
                          <p className="text-gray-300">Seller: {product.product.seller?.username}</p>
                        </div>
                      </div>
                    ))}
                  </div>
